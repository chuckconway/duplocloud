...
from src.intents.message_classifier import MessageClassifierIntent
...
def action_list() -> list[(str, lambda emit: None)]:
    return [
        ...
        (RAGIntent.action_name, lambda emit, message, session: RAGIntent(emit=emit, message=message, session=session)),
        (MessageClassifierIntent.action_name, lambda emit, message, session: MessageClassifierIntent(emit=emit, message=message, session=session)),
